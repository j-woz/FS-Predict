...
. `$ spack repo add ~/proj/spack/mochi-spack-packages`
. `$ spack repo list`
...
image::https://github.com/j-woz/FS-Predict/blob/prediction/mofka/workflow.png[]

. Beamline Data is generated and copied across filesystems from Chirp to Chirp to Lustre.
. Trace data from Chirp is collected into logs that the Monitor reads.
. The Predictor trains on historical and emerging data.
. The Predictor periodically posts performance predictions to Mofka.
. The Apps read/write/read the Beamline Data, performing some scientific task.
. (Near term) The Agent controlling the Apps can consume performance predictions from Mofka and use the predictions to tune App behavior.  What can we measure here to show success?
. (Future) The Predictor performance predictions can be read from Octopus by the Beamline Data Generator, and use to tune Generation behavior (such as throttling data capture rate, reducing resolution, etc.).
